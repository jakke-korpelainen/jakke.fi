...
export const WorkTime = () => {
  ...
  const pauses = [...state.pauses].sort((a, b) =>
    sortDateDesc(a.startTime, b.startTime),
  );
    ...
    <div className="flex grow flex-col text-white lg:flex-row">
      <Column className={clsx(columnClass, "grow bg-black lg:mr-[50%]")}>
        ...
        {pauses.map((pause, index) => (
          <div
            className="relative rounded border border-orange-900 p-5"
            key={pause.id}
          >
            <h3 className="mb-5">
            ...
            <TimeInput
              ...
              onStartChange={(value) =>
                dispatch({ type: "SET_PAUSE_START_TIME", id: pause.id, value })
              }
              onEndChange={(value) =>
                dispatch({ type: "SET_PAUSE_END_TIME", id: pause.id, value })
              }
              onPaidChange={(value) =>
                dispatch({ type: "SET_PAUSE_PAID", id: pause.id, value })
              }
            />
          </div>
        ))}
      ...
      <Column
        className={clsx(
          columnClass,
          "lg:min-h-auto order-first min-h-[20vh] items-center justify-center bg-orange-900 lg:fixed lg:right-0 lg:order-none lg:h-full",
        )}
      >
        <ClockOut {...state} />
      </Column>
